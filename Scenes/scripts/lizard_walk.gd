...
@onready var lizard = $lizard  # adjust to wherever you instance it

func _ready():
	lizard.z_index = 7

	if not Global.lizard_escaped_changed.is_connected(_on_escaped):
		Global.lizard_escaped_changed.connect(_on_escaped)


func _on_escaped():
	if lizard:
		lizard.start_wandering()
		lizard.timer.start()  # kick off wandering
