...
func _ready() -> void:
	...
	# Must wait for NavigationServer to sync the map
	await get_tree().physics_frame
	await get_tree().physics_frame  # two frames is safer in Godot 4

	# Debug: confirm navmesh is live
...
# Called by navigation.gd (your Animals script) when escape signal fires
